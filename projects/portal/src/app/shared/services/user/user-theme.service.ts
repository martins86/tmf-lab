...
export class UserThemeService {
	...
	toggleThemes(toggleControl: FormControl, overlayElement: HTMLElement): void {
		toggleControl.valueChanges.subscribe((isDark: boolean) => {
			if (isDark) {
				this.switchClass(this.darkClassName, this.lightClassName, overlayElement);
				this.userDefinitionsService.updateDefinitionItem('theme', this.darkClassName);
			} else {
				this.switchClass(this.lightClassName, this.darkClassName, overlayElement);
				this.userDefinitionsService.updateDefinitionItem('theme', this.lightClassName);
			}
		});
	}
	...
	switchClass(addClass: string, removeClass: string, overlayElement?: HTMLElement): void {
		if (overlayElement) {
